...
select a.issued_member_id , b.member_name , a.issued_book_name,
(CURRENT_DATE - a.issued_date) as Overdue_days  from 
members b , issued_status a where a.issued_member_id = b.member_id and 
...
select a.emp_name, count(b.issued_book_name) as no_of_books_processed,c.branch_address FROM 
    employees a
JOIN 
    issued_status b ON b.issued_emp_id = a.emp_id
JOIN 
    branch c ON a.branch_id = c.branch_id
GROUP BY 
    a.emp_name, c.branch_address
ORDER BY 
    no_of_books_processed DESC
LIMIT 3;


/*Task 18: Identify Members Issuing High-Risk Books
Write a query to identify members who have issued books more than twice with the 
status "damaged" in the books table. Display the member name, book title, 
and the number of times they've issued damaged books.*/

SELECT 
    m.member_name, 
    b.book_title, 
    COUNT(ib.issued_book_name) AS times_issued_damaged
FROM 
    members m
JOIN 
    issued_status ib ON m.member_id = ib.issued_member_id
JOIN 
    books b ON ib.issued_book_isbn = b.isbn
WHERE 
    b.status = 'damaged'
GROUP BY 
    m.member_name, b.book_title
HAVING 
    COUNT(ib.issued_book_name) > 2;
...
SELECT 
    m.member_id,
    COUNT(r.return_id IS NULL) AS number_of_overdue_books,
...
WHERE
    r.return_id IS NULL  -- Only books that have not been returned
    AND (CURRENT_DATE - i.issued_date) - 30 > 0  -- Overdue by more than 30 days
GROUP BY 
    m.member_id;
